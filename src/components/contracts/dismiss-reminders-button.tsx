...
export default function DismissRemindersButton({ contractId }: DismissRemindersButtonProps) {
  ...
  const handleDismiss = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        toast.error(data.error || 'Could not dismiss reminders.')
        return
      }
    ...
    return (
      <div className="rounded-xl border-2 border-orange-100 bg-orange-50/40 p-4 space-y-3">
        <p className="text-xs font-bold text-gray-700 leading-relaxed">
          Stop all expiry reminders for this contract? This can't be undone.
        </p>
