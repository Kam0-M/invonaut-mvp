...
import {
  ...
  Loader2, BookOpen, X, Check, AlertTriangle,
} from 'lucide-react'
import { toast } from 'sonner'

type ClauseBlock = { id: string; title: string; content: string; category: string }
            ...
            filtered.map(clause => (
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  <button
                    onClick={() =>
                      onAdd({ title: clause.title, content: clause.content, category: clause.category })
                    }
                ...
                <p className="text-xs text-gray-500 line-clamp-2 leading-relaxed">{clause.content}</p>
              </div>
            ))
          )}
        </div>
...
export default function EditContractPage({
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [contractRes, clientsRes, clausesRes] = await Promise.all([
        supabase
          .from('contracts')
          .select('id, title, status, template_type, content, client_id, total_value, start_date, end_date, expiry_date')
          .eq('id', id)
          .eq('user_id', user.id)
          .single(),
        supabase
      ...
      const loadedClauses: ClauseBlock[] = ((c.content ?? []) as any[]).map(
        (cl: any, i: number) => ({
      ...
      setContractId(c.id)
      setIsRenewal(c.title.includes('(Renewal)'))
      setTitle(c.title)
      setClientId(c.client_id ?? '')
      setTemplateType(c.template_type ?? 'custom')
      setTotalValue(c.total_value != null ? String(c.total_value) : '')
      setStartDate(c.start_date ?? '')
      setEndDate(c.end_date ?? '')
      setExpiryDate(c.expiry_date ?? '')
      setClauses(loadedClauses)
      setExpandedClauses(new Set(loadedClauses.map(cl => cl.id)))
        ...
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4">
          ...
          <div>
            ...
            <p className="text-amber-700 text-sm mt-0.5 leading-relaxed">
              Your client will sign this version fresh — any clause changes you make here will become
              the new agreed terms. Make sure any updates are intentional before sending.
            </p>
          </div>
        </div>
      )}
